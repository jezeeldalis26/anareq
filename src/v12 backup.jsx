    ...
    try {
      if (!window.html2canvas) {
        await new Promise((resolve, reject) => {
          const existing = document.querySelector('script[data-anareq-html2canvas]');
          if (existing) { existing.addEventListener('load', resolve, { once: true }); existing.addEventListener('error', reject, { once: true }); return; }
          const script = document.createElement('script');
          script.dataset.anareqHtml2canvas = 'true';
      ...
      if (!window.jspdf) {
        await new Promise((resolve, reject) => {
          const existing = document.querySelector('script[data-anareq-jspdf]');
          if (existing) { existing.addEventListener('load', resolve, { once: true }); existing.addEventListener('error', reject, { once: true }); return; }
          const script = document.createElement('script');
          script.dataset.anareqJspdf = 'true';
          script.src = 'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js';
          script.onload = resolve; script.onerror = reject; document.head.appendChild(script);
        });
      }
      element.classList.add('pdf-render-mode');
      const pages = Array.from(element.querySelectorAll('.professional-pdf-page'));
      if (pages.length === 0) throw new Error('PDF template pages not found');
      ...
      for (let index = 0; index < pages.length; index += 1) {
        const canvas = await window.html2canvas(pages[index], { scale: 2, useCORS: true, backgroundColor: '#ffffff', logging: false, imageTimeout: 15000, windowWidth: 794 });
        const imgData = canvas.toDataURL('image/jpeg', 0.94);
        let imgWidth = maxWidth;
        let imgHeight = (canvas.height * imgWidth) / canvas.width;
        if (imgHeight > maxHeight) { const ratio = maxHeight / imgHeight; imgHeight = maxHeight; imgWidth *= ratio; }
      ...
      showToastMessage('PDF', languageCode === 'en' ? 'The PDF could not be generated. Please try again.' : languageCode === 'pt' ? 'Não foi possível gerar o PDF. Tente novamente.' : 'No se pudo generar el PDF. Intenta nuevamente.');
    } finally {
...
*${t('conclusion')}:* ${res.summary}`;
  ...
  const shareFromHistory = (item) => {
    ...
    showToastMessage(t('copied'), t('copiedDesc'));
  };
  ...
  return (
    <div className="min-h-screen bg-[#f4f2f0] text-stone-800 font-sans selection:bg-orange-200 selection:text-orange-900 relative">
              ...
              <div className="w-full max-w-6xl mx-auto space-y-6 no-print">
                ...
                <div className="sticky top-20 z-20 bg-stone-900 rounded-2xl p-3.5 text-white shadow-lg border border-stone-800">
                  ...
                  <div className="grid grid-cols-4 gap-1 divide-x divide-stone-700">
                    ...
                    <div className="text-center px-1"><p className="text-[8px] text-stone-400 font-bold uppercase">{t('leads')}</p><p className="text-xs font-black">{safeNum(totalLeads).toLocaleString()}</p></div>
                        ...
                        <tr key={item.id} className="border-b border-stone-100 hover:bg-stone-50 transition-colors group">
                          ...
                          <td className="py-4 px-4 text-right">
                            <div className="flex justify-end gap-2">
                               <button onClick={() => shareFromHistory(item)} className="p-2 bg-white border border-stone-200 rounded-lg text-stone-500 hover:text-orange-600 hover:border-orange-200 shadow-sm transition-colors" title={t('copySummary')}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .professional-pdf-stage { position: absolute; left: -12000px; top: 0; width: 794px; pointer-events: none; z-index: 0; }
